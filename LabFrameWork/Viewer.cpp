...
void Viewer::getFrustrumInfo() {
	...
	if (glm::length(m_imagePlaneVertDir) != 0)
	...
	if (glm::length(m_imagePlaneHorizDir) != 0)
...
glm::mat4 Viewer::lookAt(glm::vec3 pos, glm::vec3 look, glm::vec3 up)
{
...
glm::mat4 Viewer::perspective(float fov, float aspect, float n, float f)
{
	glm::mat4 P(0.0f);
